...
'''
Edited by: 
Devon Zhang /  Will Zhao
Date: 2020.9.10
CSCI 204 Project 1 Phase 2
Instructor: Professor Guteknust
'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        #Compute the total inventory value
        if data['action'] == 'compute_inventory':
            ret_text = home_page + '\n' + '<p>' + home_url + '<p>' + inventory.compute_inventory()                  
        #List all the items that the user wants to see
        elif data['action'] == 'print_inventory':
            #check if the key 'Parameter2' has value, if it does,this means \
            #the user wants a specific range of list of items, then print the 
            #partial list of items; if not, print all the list of items
            if data['Parameter2'] != '':
                parameter = data['Parameter2']
                parameter = parameter.split(',')
                par1 = int(parameter[0])              
                par2 = int(parameter[1])
                ret_text = home_page + '\n'+ '<p>' + home_url + '<p>'  + inventory.print_inventory(par1, par2)                  
            else:
                ret_text = home_page + '\n' + '<p>' + home_url + '<p>'  + inventory.print_inventory()                  
                  
        #Search and list all items by word pattern     
        elif data['action'] == 'search':
            #'par' represents a string that the user inputed. When the user \
            # inputs something, it will search and list all items if the title\
            # of that item contains this word
            par = data['Parameter2']
            ret_text = home_page + '\n' + '<p>' + home_url + '<p>' + inventory.search_item(par)                  
